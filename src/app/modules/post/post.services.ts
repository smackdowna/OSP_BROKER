...
const createPost = async (post: TPost, res: Response, req: Request) => {
  ...
  if (media) {
    ...
    const businessFollowers = await prismadb.businessPageFollower.findMany({
      where: {
      ...
      await Promise.all(
        businessFollowers.map(async (follower) => {
            notifyUser(follower.user.id, {
              type: "POST",
              message: `${business?.businessName} created a new post ${newPost.title}`,

...
const createCategory = catchAsyncError(
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        const category = await categoriesServices.createCategory({name , description  , icon, membership_access});
        sendResponse(res, {
            statusCode: 200,
            success: true,
...
const softDeleteCategory = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const deletedCategory = await categoriesServices.softDeleteCategory(id , res);
    sendResponse(res, {
        statusCode: 200,
        success: true,
